"" "
  Módulo principal - instancia objetos de classes definidas em
                módulos do pacote projeto01.
"" "
de  clienteNotaFiscal  import  Cliente
de  itemNotaFiscal  importar  ItemNotaFiscal
de  notaFiscal  importação  NotaFiscal
from  product  import  Produto


def  main ():
    cli  =  Cliente ( 1 , "Manoel Costa" , 100 , "200.100.345-34" )

    p1  =  Produto ( 1 , 100 , "Arroz Branco" , 5.5 )
    IT1  =  ItemNotaFiscal ( 1 , 1 , 10 , p1 )

    p2  =  Produto ( 2 , 200 , "Feijão de Corda" , 8,5 )
    it2  =  ItemNotaFiscal ( 2 , 2 , 10 , p2 )

    p3  =  Produto ( 3 , 300 , "Macarrão Ninho" , 4,5 )
    it3  =  ItemNotaFiscal ( 3 , 3 , 10 , p3 )

    nf  =  NotaFiscal ( 1 , 100 , cli )

    nf . adicionarItem ( it1 )

    nf . adicionarItem ( it2 )

    nf . adicionarItem ( it3 )

    nf . calcularNotaFiscal ()

    imprimir ( "Valor Total
   ="  +  str ( nf . valorNota ))

    nf . imprimirNotaFiscal ()

if  __name__  ==  '__main__' :
    principal ()